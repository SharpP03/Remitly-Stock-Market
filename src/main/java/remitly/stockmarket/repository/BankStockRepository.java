...
public interface BankStockRepository extends JpaRepository<BankStock, Long> {
    ...
    @Query("select bs from BankStock bs where bs.bank = :bank and bs.stock = :stock")
    Optional<BankStock> findByBankAndStockForUpdate(@Param("bank") Bank bank, @Param("stock") Stock stock);
